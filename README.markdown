...
# Running tests on a mac (M1)

For development on a mac the `realpath` program needs to be installed:

`brew install coreutils`

After this is installed run the `pbjs.sh` script

```./integration/pbjs.sh````

This will add `pbjs.js` & `pbjs.d.ts` files to the correct integration folders and allows `jest` to run the tests.
